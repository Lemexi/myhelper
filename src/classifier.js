...
return /(?:^|\s)(?:я\s+бы\s+ответил[ао]?|я\s+ответил[ао]?\s+бы|ответил[ао]?\s+бы)(?:\s|$)/i.test(t);
}

export function parseCmdTeach(raw = “”) {
const t = stripQuoted(raw);
// Исправленное регулярное выражение с более точным захватом текста
const re = /(?:я\s+бы\s+ответил[ао]?|я\s+ответил[ао]?\s+бы|ответил[ао]?\s+бы)\s*:?\s*([\s\S]+?)$/i;
const m = t.match(re);
