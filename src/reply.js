...
import {
  ...
  isCmdAnswerExpensive, honorific, extractGreeting
} from "./classifier.js";
...
/* ----- Просьба имени (зеркалим приветствие) ----- */
...
export async function smartReply(sessionKey, channel, userTextRaw, userLangHint = "ru") {
  ...
  // Узнаём имя из БД, но учитываем «имя найдено в текущем сообщении»
  const session = await getSession(sessionId);
  const knownName = nameInThisMsg || session?.user_name?.trim();

  // Если имени всё ещё нет → зеркалим приветствие и культурно просим имя (один раз)
  if (!knownName) {
  ...
  // Обращение (если вдруг захочешь использовать Sir/Ma’am при отсутствии имени — сейчас имя уже есть)
  const finalAnswer = answer;

  // Сохраняем исход как EN
