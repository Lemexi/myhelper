...
export async function upsertSession(sessionKey, channel) {
  const { rows } = await pool.query('SELECT id FROM sessions WHERE session_key=$1 LIMIT 1',[sessionKey]);
...
export async function saveMessage(sessionId, role, content, meta=null, lang=null, translated_to=null, translated_content=null, category=null) {
  try{
    ...
    `;
    const { rows }=await pool.query(q,[sessionId, role, content, meta?JSON.stringify(meta):null, lang, translated_to, translated_content, category]);
...
export async function loadRecentMessages(sessionId, limit=24){
  const { rows } = await pool.query('SELECT role, content FROM messages WHERE session_id=$1 ORDER BY id DESC LIMIT $2',[sessionId, limit]);
  return rows.reverse().map(r=>({role:r.role, content:r.content}));
}
export async function loadLatestSummary(sessionId){
  const { rows }=await pool.query('SELECT content FROM summaries WHERE session_id=$1 ORDER BY id DESC LIMIT 1',[sessionId]);
...
export async function logReply(sessionId, strategy, category, kbItemId, messageId=null, notes=null){
  ...
  await pool.query(`
    ...
    VALUES ($1,$2,$3,$4,$5,$6)
  `,[sessionId, safe, category, kbItemId, messageId, notes]);
}
export async function getLastAuditCategory(sessionId){
  const { rows }=await pool.query(`SELECT category FROM reply_audit WHERE session_id=$1 AND category IS NOT NULL ORDER BY id DESC LIMIT 1`,[sessionId]);
...
async function ensureCorrectionsTable(){
  ...
  CREATE TABLE IF NOT EXISTS corrections (
    ...
    category text,
    trigger_user_en text,          -- ← на что отвечали
    prev_answer_en text NOT NULL,  -- ← что ответил бот
    taught_en text NOT NULL,       -- ← как надо отвечать
    taught_local text,
    taught_lang text,
